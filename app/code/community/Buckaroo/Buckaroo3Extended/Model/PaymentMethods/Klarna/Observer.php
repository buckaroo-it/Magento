 ...
 */
class Buckaroo_Buckaroo3Extended_Model_PaymentMethods_Klarna_Observer extends Buckaroo_Buckaroo3Extended_Model_Observer_Abstract
{
    /** @var string $_code */
    ...
    public function buckaroo3extended_response_custom_processing(Varien_Event_Observer $observer)
    {
        ...
        if ($responseObject->Status->Code->Code == '791') {
            ...
            $status = $helper->getNewStates(
                Buckaroo_Buckaroo3Extended_Helper_Data::BUCKAROO_PENDING_PAYMENT,
                $order,
                $method
            );
    ...
    public function buckaroo3extended_push_custom_processing(Varien_Event_Observer $observer)
    {
        ...
        if ($response['status'] == Buckaroo_Buckaroo3Extended_Helper_Data::BUCKAROO_REJECTED) {
            /** @var Mage_Sales_Model_Order $order */
            $order = $observer->getOrder();
            $method = $order->getPayment()->getMethod();
            $helper = Mage::helper('buckaroo3extended');
        ...
        if ($response['status'] == Buckaroo_Buckaroo3Extended_Helper_Data::BUCKAROO_SUCCESS ||
            ...
            if (!empty($responseObject['brq_SERVICE_klarnakp_AutoPayTransactionKey']) && ($responseObject['brq_statuscode'] == 190)) {
                ...
                $createInvoice = new Buckaroo_Buckaroo3Extended_Model_Observer_KlarnaCreateInvoice();
